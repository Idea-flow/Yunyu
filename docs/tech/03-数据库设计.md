...
- `(user_id, status, deleted, updated_at DESC, id DESC)`
...
- `(post_id, status, deleted, root_id, created_at, id)`
...
CREATE TABLE `user` (
  ...
  KEY `idx_user_status_deleted_created_at` (`status`, `deleted`, `created_at`, `id`),
...
- `status + deleted + created_at` 便于后台筛选用户列表
...
CREATE TABLE `tag` (
  ...
  KEY `idx_tag_status_deleted_created_at` (`status`, `deleted`, `created_at`, `id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='文章标签表';
```

设计说明：

- 标签更偏运营管理，因此以 `status + deleted + created_at` 支撑后台列表
...
CREATE TABLE `post` (
  ...
  KEY `idx_post_user_manage` (`user_id`, `status`, `deleted`, `updated_at` DESC, `id` DESC),
...
CREATE TABLE `post_tag` (
  ...
  KEY `idx_post_tag_tag_id_post_id` (`tag_id`, `post_id`)
...
CREATE TABLE `comment` (
  ...
  `deleted` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否删除：0否 1是',
  PRIMARY KEY (`id`),
  KEY `idx_comment_post_status_root_created` (`post_id`, `status`, `deleted`, `root_id`, `created_at`, `id`),
  KEY `idx_comment_user_status_created` (`user_id`, `status`, `created_at`, `id`),
...
- 评论展示主链路按 `post_id + status + deleted + root_id` 查询
