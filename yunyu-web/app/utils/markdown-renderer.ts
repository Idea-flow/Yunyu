...
/**
 * Markdown 渲染结果类型。
 * 作用：统一承接文章编辑、服务端渲染和后台 API 调用所需的 HTML、目录、纯文本和阅读时长信息。
 */
...
/**
 * 创建 Markdown 解析器。
 * 作用：为文章正文渲染提供统一的 Markdown 解析、标题锚点和基础排版能力。
 */
...
/**
 * 渲染 Markdown 内容。
 * 作用：统一生成文章编辑与 API 操作所需的 HTML、目录、纯文本与阅读时长。
 * 此函数不依赖 Vue，可在客户端 composable 和服务端 API route 中共享使用。
 *
