...
public class AdminPostCreateRequest {
    ...
    @Schema(description = "文章标题。新增时必填。", example = "Spring Boot 与 Nuxt 联调记录")
    @NotBlank(message = "标题不能为空")
    @Size(max = 200, message = "标题长度不能超过200个字符")
    private String title;

    @Schema(description = "文章唯一 slug。建议根据标题或正文语义生成，使用小写英文和连字符。", example = "spring-boot-nuxt-integration-notes")
    @NotBlank(message = "Slug不能为空")
    @Size(max = 220, message = "Slug长度不能超过220个字符")
    private String slug;

    @Schema(description = "文章摘要。可由 AI 根据正文自动生成。", example = "记录 Spring Boot 后端与 Nuxt 前端联调过程中的接口联调、鉴权和部署注意事项。")
    ...
    @Schema(description = "所属分类 ID。可为空；如需填写，应先查询可用分类列表。", example = "12")
    private Long categoryId;

    @Schema(description = "标签 ID 列表。新增文章前应先查询可用标签。", example = "[3,8]")
    private List<Long> tagIds;

    @Schema(description = "专题 ID 列表。新增文章前应先查询可用专题。", example = "[5]")
    private List<Long> topicIds;

    @Schema(
            description = "文章状态。DRAFT=草稿，PUBLISHED=发布，OFFLINE=下线。若用户未明确要求发布，建议默认传 DRAFT。",
            example = "DRAFT",
    ...
    @Schema(description = "文章正文 Markdown。新增或更新文章时必填。", example = "# 一、背景\\n\\n本文记录 Spring Boot 与 Nuxt 的联调过程。")
