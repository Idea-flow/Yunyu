package com.ideaflow.yunyu.module.friendlink.admin.dto;
...
/**
 * 后台创建友链请求类。
 * 作用：承接后台新增友链时提交的站点信息、联系信息与审核状态字段，并在进入业务层前完成参数校验。
 */
@Data
public class AdminFriendLinkCreateRequest {

    @Schema(description = "友链站点名称。创建友链时必填。", example = "IdeaFlow")
    ...
    @Schema(description = "友链状态。PENDING=待审核，APPROVED=已通过，REJECTED=已驳回，OFFLINE=已下线。", example = "PENDING", allowableValues = {"PENDING", "APPROVED", "REJECTED", "OFFLINE"})
